...
const Layout = ({ onLogout, user, refreshUser }) => {
  ...
  const fetchTasks = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [tasksRes, usersRes] = await Promise.all([
        axios.get('http://localhost:4000/api/tasks/gp', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('http://localhost:4000/api/users', { headers: { Authorization: `Bearer ${token}` } }).catch(() => ({ data: [] })),
      ]);
  ...
  const executeDelete = useCallback(async (taskId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:4000/api/tasks/${taskId}/gp`, {
